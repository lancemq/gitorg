...
export function DocSearchPanel({ initialItems, label, locale, onClose }: DocSearchProps) {
  const pathname = usePathname();
  const router = useRouter();
  const [items, setItems] = useState<SearchDoc[]>(() => initialItems ?? []);
  const [query, setQuery] = useState("");
  const [hasLoaded, setHasLoaded] = useState(() => Boolean(initialItems?.length));
  ...
  return (
    <div className="doc-search is-open">
        <div
