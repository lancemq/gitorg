...
- Understand that Git hooks are event-triggered scripts that let you intervene at key moments (commit, push, merge)
- Enable scripts under `.git/hooks/`: remove the `.sample` suffix, add execute permission, and understand that a non-zero `pre-*` exit aborts the operation
- Use Husky + lint-staged to bring hooks under version control, and know when a client-side vs server-side hook is the right tool



## Start with a problem

You accidentally committed a `.env` file with secrets in it, and only noticed after it was already pushed. A `pre-commit` hook that scanned staged files for secrets would have blocked this before it ever happened. The question is: where do these hooks live, how do you write one, and why does it protect your machine but not necessarily your teammate's?
...
1. In a test repo, enable `pre-commit`: remove the `.git/hooks/pre-commit.sample` suffix, write a script that scans `git diff --cached` for `.env`, make it executable, then try committing a `.env` file and confirm it is blocked.
2. Write a `post-commit` hook using `git rev-parse --abbrev-ref HEAD` that bumps the version automatically on the `release` branch (see scenario 4), and observe the effect with `git log`.
3. Wire `pre-commit` into version control with Husky + lint-staged: configure lint-staged to run ESLint only on staged `*.js`, then make a commit and verify the hook still works after a fresh clone.
...
- [The Complete Guide to .gitignore](/en/concepts/git-ignore)
