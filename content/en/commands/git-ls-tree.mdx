...
`git ls-tree` lives closer to Git’s implementation model: objects, refs, and the index. It is best suited for scripting, automation, and advanced debugging rather than being your first manual tool.

## Typical use cases

- Use `git ls-tree` when you need precise control over objects, refs, or index state in a scriptable flow.
- Reach for `git ls-tree` when learning internals or validating low-level repository state during debugging.
- Use `git ls-tree` only when porcelain commands no longer express the exact operation you need.
...
- Before running `git ls-tree` in a real repo, test it in a tiny disposable repository and verify whether a safer porcelain command would do the job.
