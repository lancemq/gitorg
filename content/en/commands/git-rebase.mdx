...
- Understand the mental model of rebase: commits are replayed onto a new base, so commit IDs change while content survives
- Use `git rebase origin/main`, interactive rebase, conflict handling, and `--force-with-lease` safely
- Know when rebase is appropriate, when merge is safer, and how to recover with reflog after a mistake
...
1. On a throwaway branch, use `git rebase -i HEAD~4` to squash four noisy commits into two logical blocks, then confirm with `git log --oneline`.
2. Deliberately create a conflict mid-rebase and walk through `git status → resolve → git add → git rebase --continue`; then try `git rebase --abort` to feel the difference.
3. Push a rebased branch with both `git push --force` and `git push --force-with-lease` on a backup branch to see why the latter is safer.
...
- [git reflog](/en/commands/git-reflog)
