...
- Understand the core purpose of Parallel Task Handling with Worktree
- Master the basic usage and common options of Parallel Task Handling with Worktree
...
<CommandFlowFigure
  title="Worktree Parallel Work Mode"
  caption="Multiple worktrees of the same repo share .git directory but can checkout different branches. Suitable for handling bug fix, feature and review simultaneously."
  inputsLabel="Single Workspace Pain"
  inputs="Feature development interrupted|Need urgent bug fix|Review requires immediate changes"
  commandLabel="git worktree add"
  outputsLabel="Multi-workspace Parallel"
  outputs="../fix/ → checkout hotfix branch|../review/ → checkout review branch|Main dir continues feature dev"
  note="All worktrees share the same .git directory, commits and objects are shared. Deleting worktree doesn't affect commit history."
/>
...
1. Practice the parallel-work-with-worktree command in a test repository and observe state changes before and after
