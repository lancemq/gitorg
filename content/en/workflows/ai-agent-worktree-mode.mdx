...
- Understand the core purpose of Using git worktree as the default mode for AI coding agents
- Master the basic usage and common options of Using git worktree as the default mode for AI coding agents
...
- Understand key concepts: What this topic solves
...
<CommandFlowFigure
  title="AI Agent + Worktree Parallel Mode"
  caption="Each worktree corresponds to an independent branch and Agent task, avoiding context pollution. One worktree handles bug fix, another continues feature development."
  inputsLabel="Multi-task Scenarios"
  inputs="Bug fix task|Feature A development|Code review changes|Agent experiments"
  commandLabel="git worktree management"
  outputsLabel="Independent Workspaces"
  outputs="wt-bugfix/ → fix/login-timeout|wt-feature/ → feature/ai-integration|wt-review/ → review/comments|Each worktree: independent branch, independent context"
  note="AI agent runs in worktree without interfering with your main workspace. Delete worktree when task is done."
/>
...
1. [`git worktree`](/en/commands/git-worktree)
2. [`parallel work with worktree`](/en/workflows/parallel-work-with-worktree)
3. [`prepare commits before pull request`](/en/workflows/prepare-commits-before-pull-request)
4. [`hotfix and urgent fixes`](/en/workflows/hotfix-and-urgent-fixes)
