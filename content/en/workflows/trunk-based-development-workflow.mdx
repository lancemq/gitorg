...
- Understand the core discipline of TBD: short branch lifetime and frequent small merges keep main releasable
- Apply the operating cadence: cut short branches from main, commit in small batches, rebase/merge main, then merge fast
- Compare TBD with GitFlow and recognize the "short-branch" traps that masquerade as TBD
...
You just merged a feature branch that lived for two weeks. Conflict resolution ate a full day, and CI went red because of someone else's change. All that time main kept moving while your branch sat in isolation. Had you sliced the work small and returned to main daily, today's merge nightmare would have been a dozen near-conflict-free syncs.
...
1. Take a recent large feature and split it into 3–6 independently mergeable batches, defining the minimum rollback unit for each.
2. In a test repo, simulate the TBD cadence: cut a short branch from main, use `git add -p` for small commits, then `git rebase origin/main` before merge, and feel how shorter divergence means fewer conflicts.
3. Pick a case where your team forbids rebasing, design the equivalent "platform updates branch on PR" flow, and write the matching CI gate checklist.
