...
- Understand the core purpose of CI/CD Git optimization
- Master the basic usage and common options of CI/CD Git optimization
...
- Understand key concepts: Why Git operations are slow in CI/CD
...
<CommandFlowFigure
  title="CI/CD Git Optimization Flow"
  caption="Git optimization strategies in CI: shallow clone reduces download, caching .git directory speeds up subsequent runs, partial clone fetches objects on demand."
  inputsLabel="Full Clone"
  inputs="Download full history (.git/)|Extract all objects|Build workspace"
  commandLabel="CI Optimization"
  outputsLabel="Optimization Result"
  outputs="--depth 1 shallow clone|Cache .git directory|Partial clone on-demand|Only pull changed files"
  note="CI/CD usually only needs latest code, not full history. Shallow clone can reduce download time by 80-95%."
/>
...
1. Practice the ci-optimization-with-git command in a test repository and observe state changes before and after
