...
- Understand the right order for learning Git: build a working loop first, then go deep
- Know the full path from environment setup, staging and commit, remote sync, to your first feature branch
- Build the habit of inspecting state before acting
- Know which high-risk topics (rebase, reset --hard, reflog recovery) to leave until the basics feel stable
...
1. **Run the minimum loop**: create a local repo and walk through `git init` → edit → `git add` → `git commit` → `git log --oneline` until the "edit → commit" step feels automatic.
2. **Build the inspect-first habit**: before every commit, run `git status` and `git log --oneline --decorate -5` for three days in a row, and notice how it prevents mistakes.
3. **Read the four pages in order**: go from Setup and Clone to First Feature Branch, and actually do the steps in your own test repo after each page instead of reading them all at once.
...
- [Staging and Commit](/en/learning-path/stage-and-commit)
