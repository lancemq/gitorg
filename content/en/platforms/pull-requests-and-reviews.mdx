...
<CommandFlowFigure
  ...
  caption="Pull Request is the core code review tool. From creating PR, automated checks, peer review to final merge, each stage has best practices."
  inputsLabel="PR Creation"
  inputs="Feature branch ready|PR description complete|Linked Issue"
  commandLabel="PR Review"
  outputsLabel="Merge or Revise"
  outputs="CI auto-checks|Peer code review|Request changes or approve|Merge to target branch"
  note="Good PR descriptions include: change purpose, implementation approach, test coverage, screenshots (if applicable), linked Issues."
/>
...
1. Practice the pull-requests-and-reviews command in a test repository and observe state changes before and after
