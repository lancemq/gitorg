...
<CommandFlowFigure
  title="Tree Directory Snapshot Organization"
  caption="Root trees point to sub-trees and blobs, forming a complete directory hierarchy snapshot. Commits ultimately point to root trees to express the project's current state."
  inputsLabel="Directory Structure"
  inputs="src/app.ts → blob:app_hash|src/utils.ts → blob:utils_hash|README.md → blob:readme_hash|tests/ → sub-tree"
  commandLabel="root tree organizes"
  outputsLabel="Snapshot Expression"
  outputs="tree: src/ (with app, utils)|tree: tests/ (with test files)|tree: root dir (with README, src/, tests/)"
  note="Each commit points to a complete tree snapshot, not a diff. Git ensures storage efficiency through object reuse."
/>
...
1. Practice the tree-objects-and-snapshots command in a test repository and observe state changes before and after
