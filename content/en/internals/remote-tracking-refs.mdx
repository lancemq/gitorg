...
- Understand the core purpose of Remote-Tracking References
- Master the basic usage and common options of Remote-Tracking References
- Understand the difference between `main` and `origin/main` so fetch, pull, push, and branch sync become easier to reason about.
- Understand key concepts: Why Git keeps them separate
...
<RefsHeadFigure
  title="Local Branch vs Remote-Tracking Ref"
  caption="main is your local branch that you can freely commit to and switch. origin/main is a snapshot of the remote state that only updates on fetch. They can diverge."
  headLabel="Normal state (main = origin/main)"
  branchLabel="main → local commit F"
  tagLabel="origin/main → remote commit D"
  remoteLabel="Remote snapshot"
  detachedLabel="Diverged state"
/>
...
1. Practice the remote-tracking-refs command in a test repository and observe state changes before and after
...
- [Refspecs and Ref Updates](/en/internals/refspec-and-ref-updates)
