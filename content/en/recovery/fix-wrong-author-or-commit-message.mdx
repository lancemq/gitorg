...
- Understand the core purpose of Fixing wrong author or commit message after commit
- Master the basic usage and common options of Fixing wrong author or commit message after commit
...
- Understand key concepts: Fixing the latest commit
...
<CommitGraphFigure
  title="Replacing Wrong Commits"
  caption="Git commits are immutable. You cannot edit an existing commit, but you can create a new one to replace it. Amend replaces the latest commit. Interactive rebase can fix commits in the middle of history."
  linearLabel="Original chain (with error)"
  mergeLabel="After amend (replaced latest)"
  rewriteLabel="After rebase -i (new chain replaces old)"
...
1. Practice the fix-wrong-author-or-commit-message command in a test repository and observe state changes before and after
...
- [Recovering commits made in detached HEAD](/en/recovery/fix-detached-head-accidentally-committed)
