export const metadata = {
  ...
  citations: [
    ...
    },
    {
      ...
      url: "https://git-scm.com/book/en/v2/Git-Tools-Reset-Demystified.html",
      kind: "book",
    },
  ],
};
...
- 理解 git reset 教程 的核心作用和适用场景
- 掌握 git reset 教程 的基本用法和常用参数
...
<ResetFigure
  title="reset 的三层影响"
  caption="不管 soft、mixed 还是 hard，都会先移动 HEAD。区别在于暂存区和工作区是否继续跟着目标提交一起重置。"
  layersLabel="影响层"
  softLabel="--soft"
  mixedLabel="--mixed"
  hardLabel="--hard"
...
1. [`git status`](/zh/commands/git-status)
2. `git log --oneline --decorate -5`
3. [`git reflog`](/zh/commands/git-reflog)
4. 如有必要先建救援分支
5. 再执行 reset
...
1. [`git status`](/zh/commands/git-status)
2. [`git reflog`](/zh/commands/git-reflog)
3. 建一个救援分支
...
- 把 `git reset` 放进“先备份、再变更、最后复核”的高风险操作流程，降低误操作损失。
...
<PracticeLab
  title="练习：比较 reset 三种模式"
  intro="这一组练习最重要的是观察 HEAD、暂存区和工作区分别发生了什么，而不是只记住参数名字。"
  setupLabel="准备环境"
  setup={`git switch -c lab/reset-demo\n# 连续做两次提交\n# 再额外修改一个文件并执行 git add`}
  stepsLabel="跟着做"
  steps="先运行 `git reset --soft HEAD~1` 并观察 `git status`。|再重复实验，改用 `git reset --mixed HEAD~1`。|最后只在实验仓库中试一次 `git reset --hard HEAD~1`。"
  outcomesLabel="结果会怎样"
  outcomes="`--soft` 会保留暂存区和工作区内容。|`--mixed` 会把暂存内容退回工作区。|`--hard` 会让 HEAD、暂存区和工作区一起回到目标提交。"
  mistakesLabel="常见错误判断"
  mistakes="如果你真正想要的是撤回提交但保留改动，通常不该直接选 `--hard`。|如果旧提交已经共享给团队，应该优先评估 `revert`，而不是直接 reset 共享历史。|只要你不确定恢复路径，先跑 `git reflog` 再继续。"
/>
...
2. [`git restore`](/zh/commands/git-restore)
3. [`git revert`](/zh/commands/git-revert)
...
- [git-rev-list 教程](/zh/commands/git-rev-list)
