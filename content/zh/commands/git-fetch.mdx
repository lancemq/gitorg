...
- 理解 git fetch 教程 的核心作用和适用场景
- 掌握 git fetch 教程 的基本用法和常用参数
- 解释 git fetch 如何更新远端引用、为何它比 pull 更可控，以及它在日常同步中的最佳位置。
- 理解 什么时候最适合先用 fetch 相关的概念
- 掌握 它到底更新了什么 相关的操作
...
<CommandFlowFigure
  ...
  outputsLabel="结果"
  outputs="新对象下载完成状态|远端跟踪引用更新"
  note="fetch 只下载不合并，不会改动你的当前分支和工作区。"
/>
...
- 和 `git log HEAD..origin/main`、`git push`、`git remote -v` 一起看，通常更容易判断问题出在同步、认证还是引用选择。
...
1. [`fetch vs pull`](/zh/workflows/fetch-vs-pull) 的策略差异
2. [`git rebase`](/zh/commands/git-rebase) 如何接住 `origin/main`
