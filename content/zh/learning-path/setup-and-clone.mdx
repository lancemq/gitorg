...
- 正确配置 Git 身份信息，让你的每次提交都有作者归属
- 理解 `git clone` 到底复制了什么——不只是文件，还有历史和远端关系
- 掌握 clone 完成后的例行检查动作：状态、分支、远端
- 区分工作区和 `.git/` 目录的职责
- 在新电脑上独立完成从零到可开发环境的搭建
...
你第一次拿到团队仓库的地址，准备开始参与项目。你在终端里敲了 `git clone`，但 clone 完之后呢？当前在哪个分支？需不需要先配点什么？仓库和本地是什么关系？——如果你对这些问题还有点模糊，这一页会帮你把所有"拿到仓库后不确定的事"理清楚。
...
1. 当前是不是在正确目录里
2. [`git status`](/zh/commands/git-status) 是否干净
3. 当前分支是不是你以为的那个分支
4. `origin` 地址是否正确
5. 当前仓库身份配置是否符合团队要求
