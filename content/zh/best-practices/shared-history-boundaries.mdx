...
- 理解 共享历史的改写边界 的核心作用和适用场景
- 掌握 共享历史的改写边界 的基本用法和常用参数
- 明确哪些历史可以放心整理，哪些历史应该默认视为共享并谨慎处理，避免 rebase、amend、reset 和 force push 伤到别人。
- 理解 这件事为什么比"会不会 rebase"更重要 相关的概念
- 掌握 1. 一个最实用的判断规则 相关的操作
...
<CommitGraphFigure
  title="共享历史边界"
  caption="一旦提交被推送到共享远端，它就成为团队共同的历史。不应该 rewrite 这部分历史。本地整理可以在推送前进行。"
  linearLabel="本地历史 (可安全 rewrite)"
  mergeLabel="已推送历史 (不应 rewrite)"
  rewriteLabel="团队共享历史 (禁止 force push)"
  pointerLabel="推送边界"
/>
...
- [小批量评审专题](/zh/best-practices/small-batch-review)
