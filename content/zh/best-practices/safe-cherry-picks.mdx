...
- 理解 安全使用 cherry-pick专题 的核心作用和适用场景
- 掌握 安全使用 cherry-pick专题 的基本用法和常用参数
- 在回移修复或选择性复用提交时，避免重复、漏拣和上下文错配。
- 理解 为什么 cherry-pick 容易"看上去简单，实际很容易出事" 相关的概念
- 掌握 1. 先确认目标分支真的缺这次提交 相关的操作
...
<CherryPickFigure
  title="安全 Cherry-Pick 实践"
  caption="Cherry-pick 适合回移修复、挑选特定提交到发布分支等场景。使用前确认提交是独立的、没有隐式依赖。"
  sourceLabel="源分支 (main)"
  targetLabel="目标分支 (release/2.x)"
  transferLabel="git cherry-pick -x <commit>"
/>

cherry-pick 表面上只是一个命令，
