...
- 建立核心认知：Git 里的提交很少被真正删除，多数是"引用路径丢了"，对象还躺在 `.git/objects/` 里
- 用 `git reflog` 找回绝大多数本地误操作（reset 过头、切走 detached HEAD、误删分支）
- 在 reflog 过期或被清理后，用 `git fsck --lost-found` 挖出 dangling 提交
- 知道哪些情况真的不可恢复（reflog 过期 + gc、克隆新仓库、远端通常无 reflog），以及提前用备份分支 / push / bundle 预防
...
<ReflogFigure
  title="提交丢失后的找回路径"
  caption="Git 中的提交很少真正被删除。大多数情况下提交对象仍然存在，只是失去了引用路径。通过 reflog 和 git fsck --lost-found 可以找到这些悬挂对象。"
  historyLabel="reflog 历史（提交操作记录）"
  rescueLabel="新建分支接住提交"
  nowLabel="当前分支位置"
/>
...
1. **制造一次 reset 过头并救回来**：在测试仓库里连续提交 5 次，然后 `git reset --hard HEAD~3`"误删"中间 3 个，用 `git reflog` 找到 reset 前的位置，`git branch recovered/reset-demo <sha>` 接住它（不要直接 `--hard` 回去，先建分支最安全）。
2. **模拟 detached HEAD 丢失**：`git checkout <老提交>` 后提交一次再切回 `main`，用 `git reflog` 找到那条"没有名字"的提交并 `git cherry-pick` 回 `main`，确认内容回来了。
3. **演练 fsck 兜底**：`git reflog expire --expire=now --all` 清空 reflog 后，再 `git fsck --lost-found` 找出 dangling commit，体会 reflog 没了之后 fsck 是最后一道网。

## 一句话保命原则

只要对象还在、reflog 没过期、你还没跑 `git gc --prune=now`——**绝大多数"丢了"的提交都还能找回来，先别慌，先用 `git reflog`**。
...
- [reflog 恢复手册](/zh/recovery/reflog-recovery)
