...
- 理解 Git Worktree 概念：多工作目录并行开发 的核心作用和适用场景
- 掌握 Git Worktree 概念：多工作目录并行开发 的基本用法和常用参数
- 系统解释 Git worktree 的概念——如何同时检出多个分支到不同目录，以及它解决的问题和适用场景。
- 理解 为什么需要 Worktree 相关的概念
- 掌握 Worktree 的核心概念 相关的操作
...
<RepositoryLayoutFigure
  title="Worktree 目录结构"
  caption="一个仓库可以同时拥有多个 worktree，每个 worktree 有独立的工作区和暂存区，但共享对象库和引用。"
  worktreeLabel="主 Worktree"
  gitdirLabel=".git 目录"
  sharedLabel="多 Worktree 共享"
  bareLabel="裸仓库"
/>
...
<WarningBox title="Worktree 不是万能方案">
...
1. [`git worktree`](/zh/commands/git-worktree) 命令的完整用法
