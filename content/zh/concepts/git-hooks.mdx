...
- 理解 Git Hooks 是“事件触发的脚本”，以及在哪些关键节点（commit/push/merge）上能介入
- 掌握 `.git/hooks/` 下脚本的存放、启用（去掉 `.sample`、加执行权限）与 `pre-*` 非零退出中断流程的机制
- 知道如何用 Husky + lint-staged 把 hooks 纳入版本控制，以及哪些场景该用客户端、哪些该用服务端钩子


## 先想一个问题

你在 `git commit` 时不小心把一个含密钥的 `.env` 提交进了仓库，等发现时已经推到了远端。如果当时有一个 `pre-commit` 钩子能在提交前扫描敏感文件并直接挡下，这场事故就不会发生。问题是：钩子到底在哪、怎么写、为什么本地能拦住却拦不住同事的机器？
...
1. 在测试仓库里启用 `pre-commit`：去掉 `.git/hooks/pre-commit.sample` 后缀，写一个检查 `git diff --cached` 是否包含 `.env` 的脚本，加执行权限后故意提交 `.env`，确认被中断。
2. 用 `git rev-parse --abbrev-ref HEAD` 写一个 `post-commit` 钩子，在 `release` 分支自动打版本号（参考本文第 4 个场景），并用 `git log` 观察效果。
3. 用 Husky + lint-staged 把 `pre-commit` 接入版本控制：配置 `lint-staged` 只对暂存区的 `*.js` 跑 ESLint，然后模拟一次提交验证钩子随仓库 clone 后仍然生效。

## 继续学习建议

1. [`git lfs`](/zh/concepts/git-lfs) —— 大文件管理
2. CI/CD 工具（GitHub Actions、Jenkins）
3. [`git bisect`](/zh/commands/git-bisect) —— 二分查找 bug
