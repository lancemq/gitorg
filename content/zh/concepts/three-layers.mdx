...
- 理解 Git 三层模型：工作区、暂存区与对象库 的核心作用和适用场景
- 掌握 Git 三层模型：工作区、暂存区与对象库 的基本用法和常用参数
- 解释 Git 最核心的三层架构——工作区（Working Tree）、暂存区（Index）与对象库（Objects Database）——以及它们如何协同工作。
- 理解 第一层：工作区（Working Tree） 相关的概念
- 掌握 第二层：暂存区（Index / Staging Area） 相关的操作
...
1. [`git add`](/zh/commands/git-add) —— 如何精确控制暂存内容
2. [`git commit`](/zh/commands/git-commit) —— 如何把暂存快照变成永久历史
3. [`git diff`](/zh/commands/git-diff) —— 如何比较不同层之间的差异
