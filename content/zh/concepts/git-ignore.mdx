...
- 理解 .gitignore 完整指南 的核心作用和适用场景
- 掌握 .gitignore 完整指南 的基本用法和常用参数
- 系统解释 .gitignore 规则语法、优先级、通配符、全局配置，以及如何排除不该提交的文件。
- 理解 为什么需要 .gitignore 相关的概念
- 掌握 .gitignore 规则语法 相关的操作
...
<CommandFlowFigure
  title=".gitignore 的匹配流程"
  caption="Git 对工作区的每个未跟踪文件按 .gitignore 规则逐行匹配，最后一条匹配的规则决定是否忽略该文件。"
  inputsLabel="工作区文件"
  inputs="src/index.js|node_modules/|.env|build/output.js|.DS_Store"
  commandLabel=".gitignore 匹配"
  outputsLabel="匹配结果"
  outputs="✅ src/index.js → 被跟踪|❌ node_modules/ → 被忽略|❌ .env → 被忽略|❌ build/ → 被忽略|❌ .DS_Store → 被忽略"
  note="最后一条匹配的规则生效。取反规则 ! 只对被忽略过的文件有效。"
/>
...
1. [`git clean`](/zh/commands/git-clean) —— 清理未跟踪文件
2. [`git attributes`](/zh/concepts/git-attributes) —— 更细粒度的文件属性控制
3. [`git sparse-checkout`](/zh/commands/git-sparse-checkout) —— 只检出部分目录
