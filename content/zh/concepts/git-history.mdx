...
- 理解 Git 历史说明 的核心作用和适用场景
- 掌握 Git 历史说明 的基本用法和常用参数
- 系统解释 Git 如何记录历史、为什么提交历史是一个有向图，以及这对协作、恢复和变更审查意味着什么。
- 理解 为什么 Git 历史这么重要 相关的概念
- 掌握 一个提交里到底有什么 相关的操作
...
<CommitGraphFigure
  title="Git 历史的三种基本形态"
  caption="Git 历史可以是简单的线性链、带汇合点的合并结构，也可以是经过重写后的新提交序列。理解这三种形态，是掌握所有历史操作的基础。"
  linearLabel="线性历史"
  mergeLabel="合并结构"
  rewriteLabel="重写后历史"
  pointerLabel="分支指向"
...
理解这张图的关键不是记住字母本身，而是看到三种不同的历史形态：**线性**（每个提交一个父节点）、**合并**（汇合点有两个父节点）、**重写**（原提交被新 ID 的提交替代）。
...
1. [`refs and HEAD`](/zh/internals/refs-and-head)
2. [`git merge`](/zh/commands/git-merge)
3. [`git rebase`](/zh/commands/git-rebase)
4. [`reflog recovery`](/zh/recovery/reflog-recovery)
