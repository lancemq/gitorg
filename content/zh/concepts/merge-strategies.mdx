...
- 理解 Git Merge 策略详解 的核心作用和适用场景
- 掌握 Git Merge 策略详解 的基本用法和常用参数
- 系统介绍 Git 的合并策略——recursive、ort、octopus、ours、subtree——它们分别解决什么问题，以及如何选择。
- 理解 Merge 策略总览 相关的概念
- 掌握 ort（Ostensibly Recursive's Twin） 相关的操作
...
<MergeFigure
  title="合并前后对比"
  caption="左：合并前两条分支各自演进；右：合并后产生 merge commit，历史结构被保留。"
  beforeLabel="合并前"
  afterLabel="合并后"
  modeLabel="git merge"
/>
...
<WarningBox title="不要对共享分支随意变更 merge 策略">
...
1. [`git merge`](/zh/commands/git-merge) 命令的完整参数
