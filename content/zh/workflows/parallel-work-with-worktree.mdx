...
- 理解 用 worktree 并行处理多个任务 的核心作用和适用场景
- 掌握 用 worktree 并行处理多个任务 的基本用法和常用参数
- 当你需要同时处理当前功能、紧急修复或评审修改时，用 git worktree 在同一个仓库上开出多个独立工作目录。
- 理解 这个工作流解决什么问题 相关的概念
- 掌握 什么时候适合用 worktree 相关的操作
...
<CommandFlowFigure
  title="Worktree 并行工作模式"
  caption="同一个仓库的多个 worktree 共享 .git 目录，但可以检出不同的分支。适合同时处理 bug fix、feature 和 review 的场景。"
  inputsLabel="单工作区痛点"
  inputs="正在开发 feature 被打断|需要紧急修复 bug|评审要求立即修改"
  commandLabel="git worktree add"
  outputsLabel="多工作区并行"
  outputs="../fix/ → 检出 hotfix 分支|../review/ → 检出 review 分支|主目录继续 feature 开发"
  note="所有 worktree 共享同一个 .git 目录，提交和对象是共享的。删除 worktree 不影响提交历史。"
/>
...
- [PR 合并策略与平台配置](/zh/workflows/pr-merge-strategy-and-platform-settings)
