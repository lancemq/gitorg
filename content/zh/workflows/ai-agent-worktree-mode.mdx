...
- 理解 AI Coding Agent 下的 git worktree 模式 的核心作用和适用场景
- 掌握 AI Coding Agent 下的 git worktree 模式 的基本用法和常用参数
- 把 git worktree 变成 AI coding agent 的默认并行工作模式，减少上下文污染、提升回滚安全性，并让多任务协作更清晰。
- 理解 这个专题解决什么问题 相关的概念
- 掌握 为什么 worktree 特别适合 AI agent 相关的操作
...
<CommandFlowFigure
  title="AI Agent + Worktree 并行模式"
  caption="每个 worktree 对应独立的分支和 Agent 任务，避免上下文污染。一个 worktree 处理 bug fix，另一个继续 feature 开发。"
  inputsLabel="多任务场景"
  inputs="Bug fix 任务|Feature A 开发|Code review 修改|Agent 实验"
  commandLabel="git worktree 管理"
  outputsLabel="独立工作环境"
  outputs="wt-bugfix/ → fix/login-timeout|wt-feature/ → feature/ai-integration|wt-review/ → review/comments|每个 worktree 独立分支、独立上下文"
  note="AI agent 在 worktree 中运行，不干扰你的主工作区。任务完成后删除 worktree 即可。"
/>
...
1. [`git worktree`](/zh/commands/git-worktree)
2. [`parallel work with worktree`](/zh/workflows/parallel-work-with-worktree)
3. [`prepare commits before pull request`](/zh/workflows/prepare-commits-before-pull-request)
4. [`hotfix and urgent fixes`](/zh/workflows/hotfix-and-urgent-fixes)
