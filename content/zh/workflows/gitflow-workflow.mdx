...
- 说清 Gitflow 里 `main` / `develop` / `feature` / `release` / `hotfix` 五类分支各自的职责与合并规则
- 理解为什么 release 和 hotfix 必须"回流"到 `main` 和 `develop` 两条线，漏掉会有什么后果
- 判断自己的团队到底适不适合 Gitflow，而不是把它当成默认最佳实践
- 区分 Gitflow 与 trunk-based 的本质差别：问题是在"分支车道"里解决，还是在"更早集成"里解决
...
不想每次手敲分支和合并，可以用社区维护的 `git-flow`（AVH 扩展）把流程固化成命令：
...
# 开一个 feature：自动从 develop 切出
git flow feature start login-audit
git flow feature finish login-audit      # 自动合并回 develop 并删除分支

# 准备发布
git flow release start 2.4.0
git flow release finish 2.4.0            # 自动合并回 main + develop 并打 tag

# 生产热修
git flow hotfix start login-timeout
git flow hotfix finish login-timeout     # 自动合并回 main + develop 并打 tag
```

扩展的价值，是把"回流 main + develop"这类容易忘的动作变成命令自动完成。但它不改变模型本身——你仍然要理解为什么这些回流必须存在，否则工具帮你合并了，你也不知道自己丢了什么。
...
2. [`release branch workflow`](/zh/workflows/release-branch-workflow)
3. [`hotfix and urgent fixes`](/zh/workflows/hotfix-and-urgent-fixes)
4. [`backport with cherry-pick`](/zh/workflows/backport-with-cherry-pick)
...
1. [`release branch workflow`](/zh/workflows/release-branch-workflow)
2. [`hotfix and urgent fixes`](/zh/workflows/hotfix-and-urgent-fixes)
3. [`shared-branch-sync-boundaries`](/zh/workflows/shared-branch-sync-boundaries)

## 给你的练习

1. **搭一套最小 Gitflow 骨架**：在测试仓库里 `git switch main` 后 `git branch develop` 并 `git push -u origin develop`，再切出 `feature/login-audit` 提交两次，最后 merge 回 `develop`，确认 `main` 始终没被直接碰。
2. **故意漏一次回流，看后果**：从 `develop` 切 `release/2.4.0`，在里面修一个 bug 并合并回 `main` + 打 tag，但故意忘了合回 `develop`；再在 `develop` 上新建功能分支，用 `git log` 确认这个修复没有进入后续开发线。
3. **用最近一次真实发布做复盘**：画出你们团队"功能开发 / 发布准备 / 线上修复"三条线，对照文章里的四个误区，判断 Gitflow 是真的解决你们的痛点，还是你们其实更需要更轻的流程。
...
- [Fork 与上游同步教程](/zh/workflows/fork-upstream-sync)
- [紧急修复工作流](/zh/workflows/hotfix-and-urgent-fixes)
- [发布后热修失败，如何回滚与稳定主线](/zh/workflows/hotfix-rollback-after-release)
